...
Splitting({ target: '.planet-title h2', by: 'chars' });
...
function updateActiveTspan() {
    const activePlanet = document.querySelector('.planet[data-active]');
    if (!activePlanet) return;

    const planetName = activePlanet.dataset.planet;

    elTspans.forEach(tspan => {
        tspan.classList.remove('active');
        if (tspan.textContent.trim().toLowerCase() === planetName) {
...
document.addEventListener("DOMContentLoaded", updateActiveTspan);
...
elTspans.forEach((tspan, i) => {
    ...
    tspan.addEventListener('click', e => {
        ...
        // Rimuove 'active' da tutti i tspan
        elTspans.forEach(ts => ts.classList.remove('active'));

        // Aggiunge 'active' al tspan selezionato
        tspan.classList.add('active');

        // Cambia il pianeta selezionato
